...
export async function loadTrustedEvaluator(modulePath, { trustedRoot = PROJECT_ROOT, fileSystem = fs } = {}) {
  ...
  return { evaluateShot: loaded.evaluateCharacterShot, proof: { id, version, implementationHash: hash(source), policyHash: hash(stable(policy)), dimensionThreshold, modulePathHash: hash(relative.replace(/\\/g, "/")) } };
}
...
export function createComfyExecutor({ compiledWorkflow, transport, clock = { sleep: (ms) => new Promise((resolve) => setTimeout(resolve, ms)) }, evaluateShot, characterContext = null, identityReferenceManifest = [], stagingRoot, transformReference, uploadReference, pollAttempts = 120, pollMs = 1000, requestTimeoutMs = 30_000, overallTimeoutMs = 15 * 60_000 } = {}) {
  ...
  return async ({ shot, character, baseUrl, signal }) => {
    ...
    try {
      ...
      const output = `${baseUrl}/view?filename=${encodeURIComponent(image.filename)}&subfolder=${encodeURIComponent(image.subfolder ?? "")}&type=${encodeURIComponent(image.type ?? "output")}`;
      const outputBytes = typeof transport.getBytes === "function" ? await transport.getBytes(output, { signal: deadline.signal, timeoutMs: requestTimeoutMs }) : null; const outputSha256 = outputBytes?.length ? hash(outputBytes) : null;
      const evaluation = typeof evaluateShot === "function" ? await evaluateShot({ output, outputSha256, shot: structuredClone(shot), character: { id: character, context: characterContext }, providerProof: structuredClone(compiledWorkflow.providerProof), references: structuredClone(prepared.references), referenceManifest: Array.isArray(identityReferenceManifest) ? identityReferenceManifest.map(safeReferenceSource) : [], execution: { promptId, terminalOutputNode: compiledWorkflow.providerProof.terminalOutputNode } }) : null;
