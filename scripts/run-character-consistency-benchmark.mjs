...
import { computeCharacterIdentityMetadataDigest, validateAndCanonicalizeCharacterIdentityMetadata } from "../src/modules/comfy-pipeline/characterIdentityMetadataRuntime.mjs";
...
export async function stageIdentityReferenceManifest(manifest, { fileSystem = fs, stagingRoot = os.tmpdir() } = {}) {
  if (!Array.isArray(manifest) || manifest.length === 0) return { manifest: Object.freeze([]), ownedRoot: null };
  await fileSystem.mkdir(stagingRoot, { recursive: true });
  const ownedRoot = await fileSystem.mkdtemp(path.join(stagingRoot, "character-benchmark-sources-"));
  try {
    const staged = [];
    for (const [index, item] of manifest.entries()) {
      const sourcePath = text(item?.sourcePath, "EREFERENCE_REQUIRED", "reference source path");
      const extension = path.extname(sourcePath).toLowerCase();
      if (![".png", ".jpg", ".jpeg", ".webp"].includes(extension)) throw fail("EREFERENCE_REQUIRED", "reference source extension is invalid");
      const targetPath = path.join(ownedRoot, `${String(index).padStart(2, "0")}-${String(item.slot ?? "reference").replace(/[^a-z0-9_-]/gi, "_")}${extension}`);
      await fileSystem.copyFile(sourcePath, targetPath, fs.constants.COPYFILE_EXCL);
      const stat = await fileSystem.stat(targetPath);
      if (!stat.isFile() || stat.size <= 0 || stat.size > MAX_REFERENCE_BYTES) throw fail("EREFERENCE_REQUIRED", "staged reference must be a non-empty regular image no larger than 40 MiB");
      const bytes = await fileSystem.readFile(targetPath);
      const format = referenceFormat(bytes); const expectedFormat = extension === ".png" ? "png" : extension === ".webp" ? "webp" : "jpeg";
      if (format !== expectedFormat) throw fail("EREFERENCE_REQUIRED", "staged reference content does not match its approved image extension");
      if (typeof fileSystem.chmod === "function") await fileSystem.chmod(targetPath, 0o444).catch(() => undefined);
      staged.push(Object.freeze({ ...item, sourceSha256: hash(bytes), sourcePath: targetPath }));
    }
    return { manifest: Object.freeze(staged.sort((left, right) => left.slot.localeCompare(right.slot))), ownedRoot };
  } catch (error) {
    await fileSystem.rm(ownedRoot, { recursive: true, force: true }).catch(() => undefined);
    throw error;
  }
}
...
export async function loadBenchmarkProjectSubject(projectPath, { character, provider, generationMode, trustedRoot = PROJECT_ROOT, fileSystem = fs } = {}) {
  ...
  const canonicalMetadata = validateAndCanonicalizeCharacterIdentityMetadata(identity);
  if (!canonicalMetadata.ok) throw fail("EIDENTITY_METADATA", canonicalMetadata.reason);
  const identityPack = { version: clean(identity.version, 80), ...canonicalMetadata.value };
  ...
  const identityMetadataDigest = computeCharacterIdentityMetadataDigest(identityPack);
  if (!identityMetadataDigest) throw fail("EIDENTITY", "matching character identity metadata is incomplete");
  const subject = { characterAssetId: character, provider, identityPackVersion: identityPack.version, identityMetadataDigest, modelName };
...
export async function runCharacterConsistencyBenchmark(options, dependencies = {}) {
  ...
  const suppliedReferenceManifest = Array.isArray(options.identityReferenceManifest) ? options.identityReferenceManifest : [];
  let canonicalReferenceSources = [];
  try { canonicalReferenceSources = canonicalizeReferenceSources(suppliedReferenceManifest); } catch { canonicalReferenceSources = []; }
  const hasSemanticReferenceManifest = suppliedReferenceManifest.some((item) => item?.slot === "face_master") && suppliedReferenceManifest.some((item) => item?.slot === "body_front");
  const stagedReferenceManifest = await stageIdentityReferenceManifest(hasSemanticReferenceManifest ? suppliedReferenceManifest : canonicalReferenceSources.length ? expandCanonicalReferenceSources(canonicalReferenceSources) : suppliedReferenceManifest, { fileSystem, stagingRoot: dependencies.sourceStagingRoot ?? dependencies.stagingRoot ?? os.tmpdir() });
  const identityReferenceManifest = stagedReferenceManifest.manifest;
  ...
  const verificationArtifactRoot = path.resolve(dependencies.artifactRoot ?? path.join(path.dirname(outputPath), `${path.basename(outputPath, path.extname(outputPath))}-artifacts`));
  report.verificationBundle = {
    schemaVersion: 1,
    artifactRoot: verificationArtifactRoot,
    outputs: [],
    identityContext: { characterAssetId: options.subject?.characterAssetId, identityPackVersion: options.subject?.identityPackVersion, identityMetadataDigest: options.subject?.identityMetadataDigest },
    references: canonicalReferenceSources.map((item) => ({ slot: item.slot, sourcePath: item.sourcePath, sourceSha256: item.sourceSha256, logicalLabel: item.logicalLabel }))
  };
  const finish = async () => { report.finishedAt = new Date(clock.now()).toISOString(); report.aggregate = summarizeBenchmark(report.shots); report.evidenceEligible = isCharacterGenerationEvidenceEligible(report); report.evidenceDigest = report.evidenceEligible ? recomputeCharacterGenerationEvidenceDigest(report) : null; try { await writeBenchmarkReportExclusive(outputPath, report, { fileSystem, randomId: dependencies.randomId }); return { exitCode: report.aggregate.status === "accepted" && report.aggregate.accepted === 8 ? 0 : 1, report, outputPath }; } finally { if (stagedReferenceManifest.ownedRoot) await fileSystem.rm(stagedReferenceManifest.ownedRoot, { recursive: true, force: true }).catch(() => undefined); } };
  ...
  for (let index = 0; index < fixture.shots.length; index += 1) {
    const shot = fixture.shots[index]; const started = clock.now();
    try {
      const raw = await executor({ shot, character, baseUrl, signal });
      const verificationOutputPath = typeof raw?.verificationOutputPath === "string" ? path.resolve(raw.verificationOutputPath) : "";
      if (verificationOutputPath && /^[a-f0-9]{64}$/i.test(raw?.outputSha256 ?? "")) report.verificationBundle.outputs.push({ id: shot.id, outputPath: verificationOutputPath, outputSha256: raw.outputSha256.toLowerCase() });
      const normalized = normalizeResult(raw, shot, compiled.providerProof, baseUrl); const referenceEvidence = normalized.referenceEvidence; delete normalized.referenceEvidence; if (referenceEvidence.length) report.references.push(...referenceEvidence); report.shots[index] = { ...emptyShot(shot), ...normalized, id: shot.id, durationMs: Math.max(0, clock.now() - started) };
    } catch (error) {
      const classified = classify(error); report.shots[index] = { ...emptyShot(shot), durationMs: Math.max(0, clock.now() - started), finalStatus: classified.code === "ECANCELLED" ? "cancelled" : "failed", failureReason: safeDiagnostic(`${classified.code}: ${classified.message}`) }; if (["ECANCELLED", "EOFFLINE", "ETRANSPORT", "EREFERENCE_REQUIRED", "EREFERENCE_TRANSFORM", "EREFERENCE_UPLOAD"].includes(classified.code)) break;
    }
  }
  return finish();
}
