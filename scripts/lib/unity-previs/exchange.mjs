...
const RELATIONS = new Set(["contact", "inside"]);
const HANDS = new Set(["none", "left", "right"]);
const ARTIFACTS = new Set(["color", "depth", "normal", "visible_mask", "isolated_mask", "openpose", "hand_pose", "contact_map"]);
const ENCODINGS = new Set(["srgb_rgb8", "linear_eye_near_white_8bit", "rh_view_normal_rgb8", "binary_visible_8bit", "binary_isolated_8bit", "openpose_body18_rgb8", "openpose_hand21_rgb8", "contact_diagnostic_rgb8"]);
...
export async function validateExport(directory, expectedScene) {
  ...
  const encodingFor = { color: "srgb_rgb8", depth: "linear_eye_near_white_8bit", normal: "rh_view_normal_rgb8", visible_mask: "binary_visible_8bit", isolated_mask: "binary_isolated_8bit", openpose: "openpose_body18_rgb8", hand_pose: "openpose_hand21_rgb8", contact_map: "contact_diagnostic_rgb8" };
  ...
  for (const [entityId, role] of entityRoles) {
    ...
    if (role === "subject") for (const kind of ["isolated_mask", "openpose", "hand_pose", "contact_map"]) if (!artifactKeys.has(`${kind}:${entityId}`)) add("artifacts", `subject artifact missing:${kind}:${entityId}`);
  }
  return { valid: errors.length === 0, errors };
}
