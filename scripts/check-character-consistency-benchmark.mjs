...
const expandedCanonicalReferences = expandCanonicalReferenceSources(canonicalizeReferenceSources([
  { slot: "front", sourcePath: "front.png", sourceSha256: "1".repeat(64) },
  { slot: "side", sourcePath: "side.png", sourceSha256: "2".repeat(64) },
  { slot: "back", sourcePath: "back.png", sourceSha256: "3".repeat(64) }
]));
assert.deepEqual(expandedCanonicalReferences.map((item) => item.slot), ["body_back", "body_front", "body_side", "expression_neutral", "face_left", "face_master", "face_right", "hair_back"], "canonical references expand to every trusted identity slot in canonical order");
assert.deepEqual(expandedCanonicalReferences.map((item) => item.logicalLabel), ["bodyBack", "bodyFront", "bodySide", "neutralExpression", "faceLeft", "faceMaster", "faceRight", "hairBack"], "canonical reference labels match the desktop trusted-context manifest");
...
const programmaticIdentityMetadataDigest = sha("c");
...
for (const [index, item] of prepManifest.entries()) await writeFile(item.sourcePath, Buffer.concat([Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]), Buffer.from(`source-${index}`)]));
...
const failedPrep = await runCharacterConsistencyBenchmark({ generationMode: "zero_shot_multi_reference", character: "asset_ada", provider: "flux2_klein_4b", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "failed-reference-upload.json"), fixture, workflow: zeroReferenceWorkflow, outputNode: "6", identityReferenceManifest: prepManifest, subject: { characterAssetId: "asset_ada", provider: "flux2_klein_4b", identityPackVersion: "identity-v3", identityMetadataDigest: programmaticIdentityMetadataDigest, modelName: "flux-2-klein-4b-fp8.safetensors" } }, {
...
let renamedUploadPromptCalls = 0; const renamedUploadRun = await runCharacterConsistencyBenchmark({ generationMode: "zero_shot_multi_reference", character: "asset_ada", provider: "flux2_klein_4b", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "renamed-reference-upload.json"), fixture, workflow: zeroReferenceWorkflow, outputNode: "6", identityReferenceManifest: prepManifest, subject: { characterAssetId: "asset_ada", provider: "flux2_klein_4b", identityPackVersion: "identity-v3", identityMetadataDigest: programmaticIdentityMetadataDigest, modelName: "flux-2-klein-4b-fp8.safetensors" } }, { transport: { ...makeTransport("flux2_klein_4b"), async postJson(...args) { renamedUploadPromptCalls += 1; return makeTransport("flux2_klein_4b").postJson(...args); } }, preflight: async () => ({ status: "ready", available: true }), evaluateShot: evaluator, evaluatorProof: programmaticEvaluatorProof, clock: testClock(), randomId: () => "fixed", transformReference: async ({ inputPath, outputPath }) => writeFile(outputPath, await readFile(inputPath)), uploadReference: async ({ subfolder }) => ({ name: "renamed.png", subfolder, type: "input" }) }); assert.equal(renamedUploadPromptCalls, 0, "renamed upload response fails before /prompt"); assert.equal(renamedUploadRun.report.evidenceEligible, false);
const zeroEvidence = await runCharacterConsistencyBenchmark({ generationMode: "zero_shot_multi_reference", character: "asset_ada", provider: "flux2_klein_4b", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "zero-evidence.json"), fixture, workflow: zeroReferenceWorkflow, outputNode: "6", identityReferenceManifest: prepManifest, subject: { characterAssetId: "asset_ada", provider: "flux2_klein_4b", identityPackVersion: "identity-v3", identityMetadataDigest: programmaticIdentityMetadataDigest, modelName: "flux-2-klein-4b-fp8.safetensors" } }, {
...
assert.deepEqual(zeroEvidence.report.verificationBundle.references.map((item) => item.slot), ["front", "side", "back"], "attestation bundle exposes the exact canonical source slots once each");
...
try {
  ...
  const first = await run("bytes-a"); const second = await run("bytes-b"); const firstStable = structuredClone(first.report); const secondStable = structuredClone(second.report); delete firstStable.verificationBundle; delete secondStable.verificationBundle; assert.deepEqual(firstStable, secondStable, "fixed inputs/clock produce identical evidence envelopes; local attestation paths are intentionally report-specific");
...
const projectAsset = { id: "asset_ada", type: "character", unrelated: "must-not-reach-evaluator", characterIdentityPack: { version: "identity-v3", triggerWord: "ada_v3", species: "human", speciesTraits: [], styleContractId: CINEMATIC_3D_DONGHUA_CONTRACT.id, styleContractVersion: CINEMATIC_3D_DONGHUA_CONTRACT.version, styleContractDigest: computeCharacterStyleContractDigest(CINEMATIC_3D_DONGHUA_CONTRACT), faceMasterPath: "refs/ada-front.png", faceLeftPath: "refs/ada-left.png", hairBackPath: "refs/ada-hair-back.png", bodyFrontPath: "refs/ada-body.png", bodySidePath: "refs/ada-body-side.png", bodyBackPath: "refs/ada-body-back.png", neutralExpressionPath: "refs/ada-expression.png", immutableTraits: ["silver bob"], forbiddenChanges: ["no eye color changes"], approvedHeroFramePaths: ["refs/ada-hero.png"], updatedAt: "2026-08-08T00:00:00.000Z" }, characterLora: { provider: "qwen_image_edit_2511", modelName: "qwen_image_edit_2511_bf16.safetensors", loraName: "ada-v3.safetensors", strength: 0.9, version: "lora-v3", status: "dataset_ready" } };
const oversizedMetadataProject = structuredClone({ assets: [projectAsset] }); oversizedMetadataProject.assets[0].characterIdentityPack.immutableTraits = Array.from({ length: 21 }, (_, index) => `trait-${index}`);
const oversizedMetadataPath = path.join(cliRoot, "oversized-metadata.json"); await writeFile(oversizedMetadataPath, JSON.stringify(oversizedMetadataProject));
await assert.rejects(loadBenchmarkProjectSubject(oversizedMetadataPath, { character: "asset_ada", provider: "flux2_klein_4b", generationMode: "zero_shot_multi_reference" }), (error) => error?.code === "EIDENTITY_METADATA", "runner rejects metadata beyond the shared canonical limits instead of silently truncating it");
const cliIdentityMetadataDigest = computeCharacterIdentityMetadataDigest(projectAsset.characterIdentityPack);
...
try {
  ...
  const cliAcceptedEnvelope = JSON.parse(await readFile(cliAcceptedPath, "utf8"));
  assert.equal(cliAcceptedEnvelope.subject.identityMetadataDigest, cliIdentityMetadataDigest, "runner and app share the canonical identity metadata digest");
  Object.defineProperty(cliAcceptedEnvelope.subject, "identityMetadataDigest", { value: cliAcceptedEnvelope.subject.identityMetadataDigest, enumerable: false, configurable: true });
  assert.equal(cliAccepted.code, 0, cliAccepted.stderr); const cliAcceptedReport = cliAcceptedEnvelope; assert.equal(cliAcceptedReport.aggregate.accepted, 8); assert.equal(cliPromptCount, 8); assert.equal(cliMaxActive, 1); assert.deepEqual(cliSeeds, fixture.shots.map((shot) => shot.seed)); assert.equal(cliAcceptedReport.evaluatorProof.version, "cli-test-v1"); assert.match(cliAcceptedReport.evaluatorProof.modulePathHash, /^[a-f0-9]{64}$/); assert.equal(cliAcceptedReport.evidenceEligible, true); assert.match(cliAcceptedReport.evidenceDigest, /^[a-f0-9]{64}$/); assert.deepEqual(cliAcceptedReport.subject, { characterAssetId: "asset_ada", provider: "qwen_image_edit_2511", identityPackVersion: "identity-v3", loraName: "ada-v3.safetensors", loraVersion: "lora-v3", modelName: "qwen_image_edit_2511_bf16.safetensors", loraStrength: 0.9, candidateStatus: "dataset_ready" }); assert.deepEqual(cliAcceptedReport.preflight.providerProof.authoritativeLoraBindings, [{ id: "4", classType: "LoraLoader", field: "lora_name", loraName: "ada-v3.safetensors", strengthModel: 0.9, strengthClip: 0.9, clipStrengthPolicy: "equal_to_model", modelPathNodeIds: ["4", "5", "2"], modelPathEdges: [{ fromNodeId: "4", fromOutputIndex: 0, toNodeId: "5", toInput: "model" }, { fromNodeId: "5", fromOutputIndex: 0, toNodeId: "2", toInput: "model" }] }]); assert.equal(validateCharacterBenchmarkEvidence(cliAcceptedReport).valid, true); const changedEnvelope = { ...cliAcceptedReport, startedAt: "later", finishedAt: "later", baseUrl: "http://secret.invalid", apiKey: "secret" }; assert.equal(recomputeCharacterBenchmarkEvidenceDigest(changedEnvelope), cliAcceptedReport.evidenceDigest, "digest excludes timestamps, endpoint, and unknown secrets");
  Object.defineProperty(cliAcceptedReport.subject, "identityMetadataDigest", { value: cliIdentityMetadataDigest, enumerable: true, configurable: true });
