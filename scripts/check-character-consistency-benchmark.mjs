...
const {
  ...
  prepareShotIdentityReferences, parseBenchmarkCliArgs, routeShotIdentityReferences,
  runCharacterConsistencyBenchmark, proveTerminalProviderWorkflow, summarizeBenchmark,
  validateBenchmarkFixture, writeBenchmarkReportExclusive
} = benchmarkRunner;
...
const prepared = await prepareShotIdentityReferences({ shot: fixture.shots[0], identityReferenceManifest: prepManifest, baseUrl: "http://127.0.0.1:8188", stagingRoot: prepRoot }, {
  ...
  uploadReference: async ({ endpoint, filename, subfolder, overwrite }) => { prepOrder.push(`upload:${filename}`); assert.equal(endpoint, "http://127.0.0.1:8188/upload/image"); assert.equal(subfolder, "character-benchmark"); assert.equal(overwrite, false); return { name: `${subfolder}/${filename}` }; }
});
...
const failedPrep = await runCharacterConsistencyBenchmark({ generationMode: "zero_shot_multi_reference", character: "asset_ada", provider: "flux2_klein_4b", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "failed-reference-upload.json"), fixture, workflow: zeroReferenceWorkflow, outputNode: "6", identityReferenceManifest: prepManifest, subject: { characterAssetId: "asset_ada", provider: "flux2_klein_4b", identityPackVersion: "identity-v3", modelName: "flux-2-klein-4b-fp8.safetensors" } }, {
  transport: { ...makeTransport("flux2_klein_4b"), async postJson(...args) { failedPromptCalls += 1; return makeTransport("flux2_klein_4b").postJson(...args); } }, preflight: async () => ({ status: "ready", available: true }), evaluateShot: evaluator, clock: testClock(), randomId: () => "fixed",
...
const zeroEvidence = await runCharacterConsistencyBenchmark({ generationMode: "zero_shot_multi_reference", character: "asset_ada", provider: "flux2_klein_4b", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "zero-evidence.json"), fixture, workflow: zeroReferenceWorkflow, outputNode: "6", identityReferenceManifest: prepManifest, subject: { characterAssetId: "asset_ada", provider: "flux2_klein_4b", identityPackVersion: "identity-v3", modelName: "flux-2-klein-4b-fp8.safetensors" } }, {
  transport: makeTransport("flux2_klein_4b"), preflight: async () => ({ status: "ready", available: true }), evaluateShot: evaluator, evaluatorProof: { id: "benchmark-test", version: "v1", implementationHash: sha("a"), policyHash: sha("b"), dimensionThreshold: 0.8 }, clock: testClock(), randomId: () => "fixed",
  transformReference: async ({ inputPath, outputPath, transform }) => writeFile(outputPath, Buffer.concat([await readFile(inputPath), Buffer.from(transform)])), uploadReference: async () => ({ ok: true })
});
...
assert.equal(zeroEvidence.report.references.length, 8, "the strict evidence envelope records one deterministic primary route per shot"); assert.equal(zeroEvidence.report.referenceManifest.some((item) => Object.hasOwn(item, "sourcePath")), false, "published manifest omits absolute paths");
...
const run = (name, provider = "qwen_image_edit_2511", evaluateShot = evaluator, extra = {}) => runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider, baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, `${name}.json`), fixture, workflow: provider === "flux2_klein_4b" ? kleinLoraWorkflow() : qwenLoraWorkflow(), workflowTokens: loraTokens, ...extra }, { transport: makeTransport(provider, { outputNode: provider === "flux2_klein_4b" ? "7" : "3" }), clock: testClock(), evaluateShot, requestTimeoutMs: 100, overallTimeoutMs: 1_000, randomId: () => "fixed" });
try {
  ...
  const explicitRun = await runCharacterConsistencyBenchmark({ ...explicitCli, fixture, workflow: explicitLoraWorkflow, workflowTokens: { CHARACTER_CONTEXT: {}, ACTIVE_LORA_NAME: "ada-v3.safetensors", ACTIVE_LORA_STRENGTH: 0.9 } }, { transport: makeTransport("qwen_image_edit_2511"), clock: testClock(), evaluateShot: evaluator, randomId: () => "fixed" });
  ...
  const throwingPreflight = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: thrownPreflightPath, fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, {
    ...
    preflight: async () => { throw circularPreflightError; }, evaluateShot: evaluator, clock: testClock(), randomId: () => "fixed"
  });
  ...
  const invalidTransport = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: invalidTransportPath, fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, {
    transport: { getJson: "not-a-function", postJson: null }, evaluateShot: async () => { invalidTransportExecuted = true; return evaluator({ providerProof: { providerId: "qwen_image_edit_2511" } }); }, clock: testClock(), randomId: () => "fixed"
  });
  ...
  const cancelledRun = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "cancelled.json"), fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, { transport: makeTransport("qwen_image_edit_2511"), evaluateShot: evaluator, clock: testClock(), signal: preCancelled.signal, randomId: () => "fixed" });
  ...
  const serialRun = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "serial.json"), fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, { transport: makeTransport("qwen_image_edit_2511", { serial }), clock: testClock(), evaluateShot: evaluator, randomId: () => "fixed" });
  ...
  for (const [name, image] of [["terminal-input", { type: "input" }], ["terminal-temp", { type: "temp" }], ["terminal-traversal", { filename: "../escape.png" }], ["terminal-absolute", { filename: "C:/escape.png" }]]) {
    const unsafeTerminal = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, `${name}.json`), fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, { transport: makeTransport("qwen_image_edit_2511", { image }), clock: testClock(), evaluateShot: evaluator, randomId: () => "fixed" });
  ...
  const continued = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "continued.json"), fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, { transport: continuationTransport, clock: testClock(), evaluateShot: evaluator, randomId: () => "fixed" });
  ...
  const offlineMidRun = await runCharacterConsistencyBenchmark({ generationMode: "lora_augmented", character: "asset_ada", provider: "qwen_image_edit_2511", baseUrl: "http://127.0.0.1:8188", outputPath: path.join(tempRoot, "offline-mid-run.json"), fixture, workflow: qwenLoraWorkflow(), workflowTokens: loraTokens }, { transport: offlineTransport, clock: testClock(), evaluateShot: evaluator, randomId: () => "fixed" });
...
const cliWorkflowPath = path.join(cliRoot, "workflow.json"); const cliEvaluatorPath = path.join(cliRoot, "evaluator.mjs"); const cliThrowEvaluatorPath = path.join(cliRoot, "throw-evaluator.mjs"); const cliWrongEvaluatorPath = path.join(cliRoot, "wrong-evaluator.mjs");
...
await writeFile(cliEvaluatorPath, `export const evaluatorVersion = "cli-test-v1";\nexport async function evaluateCharacterShot(context) { if (context.character.context?.unrelated || !context.character.context?.identityPack?.version || !context.character.context?.lora?.loraName) throw new Error("unsanitized or missing subject context"); return { status: "accepted", provenance: "model_generation", actualProvider: context.providerProof.providerId, score: 0.95, dimensionScores: { face: 0.95, hair: 0.95, outfit: 0.95, body: 0.95, quality: 0.95 }, references: [context.output], bestPreview: context.output, retries: 0 }; }\n`);
await writeFile(cliThrowEvaluatorPath, `export const evaluatorVersion = "cli-throw-v1";\nexport async function evaluateCharacterShot() { throw new Error("evaluator failed"); }\n`);
await writeFile(cliWrongEvaluatorPath, `export const evaluatorVersion = "wrong-export";\nexport default function notTheContract() {}\n`);
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && request.url?.startsWith("/view?")) { response.writeHead(200, { "content-type": "image/png" }); response.end(Buffer.from("generated-cli-image")); return; }
  if (request.method === "POST" && request.url === "/upload/image") { for await (const _chunk of request) { /* consume multipart bytes */ } return send({ name: "uploaded.png", subfolder: "character-benchmark", type: "input" }); }
