...
export function ShotNode({ shot, index, fps, selected, canMoveBack, canMoveForward, onSelect, onMove, onDragStart, onDrop }: ShotNodeProps): JSX.Element {
  const status = shot.generatedVideoPath ? "已生成" : shot.generatedImagePath ? "分镜就绪" : "待生成";
  return (
    <article data-shot-node data-selected={selected || undefined} draggable onDragStart={onDragStart} onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
