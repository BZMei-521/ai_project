...
function normalizeLayer(layer) {
  return {
    layerId: id(layer?.layerId), order: Number(layer?.order), role: String(layer?.role),
    entityIds: [...new Set((Array.isArray(layer?.entityIds) ? layer.entityIds : []).map(id))].sort(),
    artifacts: orderedArtifacts(layer?.artifacts, LAYER_ARTIFACT_KINDS)
  };
}
...
export function validateLayeredSpatialControlPack(pack, current) {
  ...
  for (const layer of Array.isArray(pack.layers) ? pack.layers : []) {
    if (!id(layer?.layerId) || layerIds.has(layer.layerId)) return { valid: false, reason: `layered_control_layer_duplicate:${layer?.layerId ?? ""}` };
    if (!Number.isFinite(layer.order) || !LAYER_ROLE_SET.has(layer.role) || !Array.isArray(layer.entityIds) || layer.entityIds.length === 0 || layer.entityIds.some((entityId) => !id(entityId))) return { valid: false, reason: `layered_control_layer_invalid:${layer.layerId}` };
    layerIds.add(layer.layerId);
    hasEnvironment ||= layer.role === "environment";
    if (layer.role === "subject") layer.entityIds.forEach((entityId) => subjectIds.add(entityId));
    const kinds = new Set();
    for (const artifact of Array.isArray(layer.artifacts) ? layer.artifacts : []) {
      if (kinds.has(artifact?.kind)) return { valid: false, reason: `layered_control_artifact_duplicate:${layer.layerId}:${artifact?.kind ?? ""}` };
      kinds.add(artifact?.kind);
      if (!isValidArtifact(artifact, LAYER_ARTIFACT_KIND_SET)) return { valid: false, reason: `layered_control_artifact_invalid:${layer.layerId}:${artifact?.kind ?? ""}` };
    }
  }
  ...
  for (const artifact of Array.isArray(pack.skeletonArtifacts) ? pack.skeletonArtifacts : []) {
    const key = `${artifact?.entityId}:${artifact?.kind}`;
    if (skeletonKeys.has(key)) return { valid: false, reason: `layered_control_skeleton_duplicate:${key}` };
    skeletonKeys.add(key);
    if (!id(artifact?.entityId) || !isValidArtifact(artifact, SKELETON_ARTIFACT_KIND_SET)) return { valid: false, reason: `layered_control_skeleton_invalid:${artifact?.entityId ?? ""}:${artifact?.kind ?? ""}` };
    skeletonEntities.add(artifact.entityId);
  }
