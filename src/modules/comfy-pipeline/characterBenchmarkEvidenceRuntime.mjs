import { validateCharacterEvidenceReceipt } from "./characterEvidenceReceiptRuntime.mjs";
import { computeCharacterIdentityMetadataDigest } from "./characterIdentityMetadataRuntime.mjs";
...
const finiteThreshold = (value) => typeof value === "number" && Number.isFinite(value) && value > 0 && value <= 1;
...
export function buildCharacterGenerationEvidencePayload(report) {
  ...
  return {
    ...
    subject: subject ? { characterAssetId: text(subject.characterAssetId), provider: text(subject.provider), identityPackVersion: text(subject.identityPackVersion), identityMetadataDigest: text(subject.identityMetadataDigest), modelName: text(subject.modelName), loraName: text(subject.loraName), loraVersion: text(subject.loraVersion), loraStrength: finiteLoraStrength(subject.loraStrength) ? subject.loraStrength : null, candidateStatus: text(subject.candidateStatus) } : null,
    ...
    workflowProof: proof,
    evaluatorProof: evaluator ? { id: text(evaluator.id), version: text(evaluator.version), implementationHash: text(evaluator.implementationHash), policyHash: text(evaluator.policyHash), dimensionThreshold: finiteThreshold(evaluator.dimensionThreshold) ? evaluator.dimensionThreshold : null } : null,
    fallbackUsed: report?.preflight?.fallbackUsed === false ? false : true,
    cutoutFallbackUsed: report?.preflight?.cutoutFallbackUsed === false ? false : true,
...
export function isCharacterGenerationEvidenceEligible(report) {
  ...
  const subjectValid = subject && [subject.characterAssetId, subject.provider, subject.identityPackVersion, subject.identityMetadataDigest, subject.modelName].every(Boolean) && sha256Value(subject.identityMetadataDigest) && subject.characterAssetId === payload.character && subject.provider === payload.provider;
  const proofValid = proof && proof.providerId === payload.provider && sha256Value(proof.workflowDigest) && Boolean(proof.terminalOutputNode) && proof.authoritativeModelBindings.length > 0 && proof.authoritativeModelBindings.every((item) => Object.values(item).every(Boolean) && basename(item.model) === basename(subject?.modelName));
  const evaluatorValid = evaluator && [evaluator.id, evaluator.version, evaluator.implementationHash, evaluator.policyHash].every(Boolean) && sha256Value(evaluator.implementationHash) && sha256Value(evaluator.policyHash) && finiteThreshold(evaluator.dimensionThreshold);
  return Boolean(core && subjectValid && proofValid && evaluatorValid && payload.fallbackUsed === false && payload.cutoutFallbackUsed === false && validReferences(payload.references) && modeHasValidLora(payload.generationMode, subject, proof) && payload.aggregate?.status === "accepted" && payload.aggregate?.accepted === 8 && finiteScore(payload.aggregate?.score) && validShots(payload.shots, payload.provider, proof.terminalOutputNode, evaluator.dimensionThreshold));
}
...
function contextReason(payload, context, mode, options = {}) {
  ...
  if (!finiteThreshold(payload.dimensionThreshold) || !finiteThreshold(context?.dimensionThreshold) || payload.dimensionThreshold !== context.dimensionThreshold) return "dimension_threshold_mismatch";
  if (mode === "lora_augmented") { for (const [reason, actual, expected] of [["lora_name_mismatch", payload.loraName, text(context?.loraName)], ["lora_version_mismatch", payload.loraVersion, text(context?.loraVersion)]]) if (!actual || !expected || actual !== expected) return reason; if (!sameStrength(payload.loraStrength, context?.loraStrength)) return "strength_mismatch"; const currentCandidateStatus = text(context?.candidateStatus); if (!CANDIDATE_LORA_STATUSES.includes(currentCandidateStatus) || payload.candidateStatus !== currentCandidateStatus) return "candidate_status_mismatch"; }
  const proof = context?.workflowProof; if (!plain(proof) || payload.workflowDigest !== text(proof.workflowDigest)) return "workflow_digest_mismatch"; if (payload.terminalOutputNode !== text(proof.terminalOutputNode)) return "workflow_terminal_mismatch"; const models = Array.isArray(proof.authoritativeModelBindings) ? proof.authoritativeModelBindings.map((item) => basename(item?.model)).filter(Boolean) : []; if (!models.length || !models.every((model) => model === basename(payload.modelName)) || basename(payload.terminalModel) !== basename(payload.modelName)) return "workflow_model_mismatch";
  return null;
}

function newStoredShapeReason(payload) {
  const required = [payload.reportLabel, payload.generationMode, payload.benchmarkVersion, payload.promptTemplateVersion, payload.fixtureDigest, payload.generationParametersDigest, payload.characterAssetId, payload.provider, payload.identityPackVersion, payload.identityMetadataDigest, payload.referenceManifestDigest, payload.modelName, payload.workflowDigest, payload.terminalOutputNode, payload.terminalModel, payload.evaluatorId, payload.evaluatorVersion, payload.evaluatorImplementationHash, payload.evaluatorPolicyHash, payload.verifiedAt, payload.importedAt];
  if (required.some((value) => !value) || !CHARACTER_GENERATION_MODES.includes(payload.generationMode) || !sha256Value(payload.fixtureDigest) || !sha256Value(payload.generationParametersDigest) || !sha256Value(payload.identityMetadataDigest) || !sha256Value(payload.referenceManifestDigest) || !sha256Value(payload.workflowDigest) || !sha256Value(payload.evaluatorImplementationHash) || !sha256Value(payload.evaluatorPolicyHash) || payload.acceptedShots !== 8 || !finiteScore(payload.aggregateScore)) return "evidence_shape_invalid";
  if (!validReferences(payload.references)) return "reference_shape_invalid";
  if (!finiteThreshold(payload.dimensionThreshold)) return "evidence_shape_invalid";
...
export function validateLegacyStoredCharacterBenchmarkEvidence(evidence, context) {
  // Legacy evidence predates authenticated receipts and is retained only for audit display.
  return { valid: false, reason: "legacy_requires_rebenchmark", expectedDigest: null, legacy: true };
}

export function validateStoredCharacterGenerationEvidence(evidence, context, options = {}) {
  ...
  const shapeReason = newStoredShapeReason(payload); if (shapeReason) return { valid: false, reason: shapeReason, expectedDigest };
  const receipt = validateCharacterEvidenceReceipt(evidence.trustedReceipt, evidence, options.trustedReceiptVerification, evidence.sourceReportDigest);
  return receipt.valid ? { valid: true, reason: "ok", expectedDigest } : { ...receipt, expectedDigest };
}
...
export function importCharacterGenerationEvidence(report, context, options = {}) {
  if (!text(report?.generationMode)) return { valid: false, reason: "legacy_requires_rebenchmark" };
