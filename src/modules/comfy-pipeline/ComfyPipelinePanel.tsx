...
export function ComfyPipelinePanel() {
  ...
  const hasUnrepairableFrontAnchorIssues = (issues: string[]) =>
    issues.some((issue) =>
      /(角色像灰模或人体模板|裸露过多或疑似裸模|主体轮廓不像标准全身角色设定图|人物过小|疑似多主体\/多角度|疑似整页小人排表|疑似场景块或群像卡片|修复结果与原始角色锚点偏差过大)/u.test(
        issue
      )
    );
  ...
  const evaluateFrontReferenceQuality = async (pathOrUrl: string, context = "") => {
    ...
    const extraMediumComponents = layout ? Math.max(0, layout.mediumComponents - 1) : 0;
    ...
    const layoutIssues = [
      ...
      layout?.significantComponents && layout.significantComponents > 1
        ...
        : "",
      layout && extraMediumComponents > 0
        ? `存在额外设定页组件(cluster=${extraMediumComponents})`
        : "",
    ...
    const layoutPenalty =
      (layout?.significantComponents && layout.significantComponents > 1
        ...
        : 0) +
      (extraMediumComponents > 0 ? 26 + extraMediumComponents * 14 : 0) +
