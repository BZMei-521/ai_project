...
The original live Klein run on 2026-08-09 completed all eight model generations with strong identity similarity but was blocked by the old full-frame edge-variance quality heuristic. A sharpened diagnostic rerun was visually rejected and never imported. The quality evaluator now measures the center subject and penalizes clipping, dense noise, and oversharpening; its clean/blur/aggressive-sharpen regression must pass before release evaluation.

Shen Yan's final `shen-yan-hybrid-v5` natural-image run uses an eight-slot manifest whose `face_master`, both profile faces, hair back, three body views, and neutral expression labels exactly match the desktop trusted context. Run v4 accepted 8/8 with aggregate `0.948297`, zero retries, and no LoRA binding. Backend artifact re-evaluation issued receipt `921fe6bb5dcc8db5e9a273cf0f021334275533b7eae89245ffd52ee558f5903b`; the production resolver selected Klein 4B with `appliedLora: null`. See `.superpowers/sdd/shen-yan-cinematic3d-release-report.md` for hashes and the remediation audit.

For every identity pack, default `species` to `human` with an empty `speciesTraits` array. Add animal ears, tails, horns, muzzles, or other beast traits only when the script explicitly marks that character as beastfolk. Do not use screenshots from films, games, social media, or unrelated generated work as canonical identity input. Migrate by generating owned front/side/back assets, reviewing them visually, recording explicit approval, versioning the identity pack, and rerunning the full eight-shot gate. Any metadata, reference-byte, provider, model, workflow, evaluator, mode, or LoRA change invalidates the corresponding stored evidence and requires a new report and receipt.
