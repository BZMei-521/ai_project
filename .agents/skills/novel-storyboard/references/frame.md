...
角色有生产身份包时，不要默认挂整张多宫格设定页。按镜头朝向选择独立的正面、左侧、右侧或背面视图；镜头方向与参考图中人物实际朝向必须一致。只有单侧视图时，可使用 manifest 中声明过 `derivedFrom` / `transform: "mirror_x"` 的派生参考；同一镜头里的脸部与身体参考必须一起镜像，不能混用相反方向。

参考图负责身份和结构，提示词还必须保证人物在目标光照下可读。侧面、逆光、夜景和棺内等高风险镜头明确写出脸部与服装保留可辨细节、主光/补光方向和禁止纯剪影；“身份相似但主体欠曝”仍是失败镜头。
...
- 角色的脸和衣服对不对得上角色设定图
- 目标左右/背面方向是否真的正确，脸部与身体参考是否同向
- 主体是否可辨；纯剪影、面部细节消失或服装黑成一块，即使身份相似也要重生成
