...
- `cameraPlan` 只帮助选择现有官方词，不把 `path / speed / amplitude` 拼成新枚举。提示词写清主体、起点、终点和速度即可。
- 一段通常只用一两个真正承担叙事任务的运动。人物动作和情绪连续优先于“高级运镜”。

## 打击帧与慢动作限制

- 打击链写成：准备/发力 → 接触 → 目标短暂迟滞 → 支撑或重心变化 → 全身结果/恢复。不要只写“猛烈打击”。
- 接触点必须和上游 `impactEvidence` 一致。若接触被遮挡，必须给可读的替代证据（目标停顿、衣甲变形、支撑脚补步、声响断点等）。
- 命中瞬间可出现**极短**画面脉冲：施力方相对清晰、受力方或接触附近短促模糊，或镜头短促抖动后立刻稳定。不要把整段写成持续晃动。
- 重叠镜头最多两段，每次必须增加新信息，例如第一次看接触、第二次看脚下支撑崩掉；禁止三连闪和无信息重复。
- 慢动作只能落在机会出现、接触完成后的短暂余波或结果阶段；禁止慢放整个发力过程。
- 真实现场音优先：破风、接触、摩擦、落地、呼吸。关键接触声不得被发力吼声、配乐或粒子音效盖住。
