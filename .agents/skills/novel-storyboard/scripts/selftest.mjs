...
const generationUpstreamQa = clone(QA_REPORT);
generationUpstreamQa.findings[0].repairScope = 'upstream';
ok(validateGenerationQa(generationUpstreamQa, correspondenceDoc).some((x) => x.includes('generation 层不能使用 upstream')), 'generation 层不得使用 upstream');
const upstreamLocalQa = clone(QA_REPORT);
upstreamLocalQa.findings[0].repairLayer = 'storyboard';
upstreamLocalQa.findings[0].repairScope = 'local-cut';
ok(validateGenerationQa(upstreamLocalQa, correspondenceDoc).some((x) => x.includes('上游问题 repairScope 必须为 upstream')), '非 generation 层不得使用 local-cut');
const generationLocalQa = clone(QA_REPORT);
generationLocalQa.findings[0].repairScope = 'local-cut';
eq(validateGenerationQa(generationLocalQa, correspondenceDoc).length, 0, 'generation 层允许 local-cut');
const upstreamUpstreamQa = clone(QA_REPORT);
upstreamUpstreamQa.findings[0].repairLayer = 'art';
upstreamUpstreamQa.findings[0].repairScope = 'upstream';
eq(validateGenerationQa(upstreamUpstreamQa, correspondenceDoc).length, 0, '上游层允许 upstream');
const numericIdQa = clone(QA_REPORT);
numericIdQa.findings[0].id = 7;
ok(validateGenerationQa(numericIdQa, correspondenceDoc).some((x) => x.includes('id 必须是非空字符串')), '数值 id 失败');
const numericTextQa = clone(QA_REPORT);
numericTextQa.findings[0].sourceRef = 7;
ok(validateGenerationQa(numericTextQa, correspondenceDoc).some((x) => x.includes('sourceRef 必须是非空字符串')), '数值文本字段失败');
const numericPreserveQa = clone(QA_REPORT);
numericPreserveQa.findings[0].preserve = [5];
ok(validateGenerationQa(numericPreserveQa, correspondenceDoc).some((x) => x.includes('preserve 项必须是非空字符串')), '数值 preserve 项失败');
const nullFindingQa = clone(QA_REPORT);
nullFindingQa.findings[0] = null;
ok(validateGenerationQa(nullFindingQa, correspondenceDoc).some((x) => x.includes('finding 必须是普通对象')), 'null finding 失败且不抛错');
